...
passport.use(new LocalStrategy({ usernameField: 'email' }, (email, password, done) => {
  User.findOne({ email: email.toLowerCase() }, (err, user) => {
    ...
    if (!user.password) {
      return done(null, false, { msg: 'Tu cuenta fue creada con un servicio, para acceder con contraseña logeate con es servicio y crea una contraseña.' });
    }
    user.comparePassword(password, (err, isMatch) => {
      ...
      return done(null, false, { msg: 'Cotraseña inválida.' });
    });
  });
}));
